...
def attack_data(model, train_data, train_labels, test_data, test_labels):
    ...
    pre_xta2 = np.reshape(np.vstack((train_labels, test_labels)), (2 * train_labels.size, 1))
    data_a = np.hstack((pre_xta1, pre_xta2))
    ## Next two lines include victim/shadow model inputs as attack model features.
    # pre_xta3 = np.vstack((train_data, test_data))
    # data_a = np.hstack((np.hstack((pre_xta1, pre_xta2)), pre_xta3))
...
def main():
    ...
    parser.add_argument('--attack_epochs', type = int, default = 30, help = 'attack epochs in NN attack')
    ...
    # Reading in the victim train and test sets, created with ces_partition.py
    ...
    # with defaults defined at beginning of main above.
    
    print("VICTIM TRAINING/EVALUATION")
    ...
    print("SHADOW TRAINING/EVALUATION")
    ...
    print("ATTACK TRAINING/EVALUATION")
    model_a = CesMiaClassifier(101,256,256,128,2)
    train_model(model_a,
                train_data_a, train_label_a, test_data_a, test_label_a,
                attack_epochs, batch_size)
